...
static ssize_t tty2tcp_poll_write(int fd, const uint8_t *buf, size_t size, unsigned timeout_msec)
{
    ...
    while ((size_t)wc < size)
    {
        ...
        if (ret <= 0)
        {
            qlog_dbg("poll(%d)=%d errno : %d (%s)\n", fd, ret, errno, strerror(errno));
            break;
        }

        if (pollfds[0].revents & (POLLERR | POLLHUP | POLLNVAL))
        {
            qlog_dbg("poll(%d) revents = %04x\n", fd, pollfds[0].revents);
            break;
        }
...
out:
    if ((size_t)wc != size)
    {
        qlog_dbg("WRAN: fd=%d, write %zd/%zd, timeout=%d\n", fd, wc, size, timeout_msec);
    }
...
{
    qlog_dbg("Usage: %s -p <tcp server port> -d <tty port> \n", self);
    qlog_dbg("Default: %s -p %d -d %s\n", self, 9000, "/dev/ttyUSB0");
}
...
int main(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "d:p:h")))
    {
        switch (opt)
        {
            ...
            case 'd': strcpy(ttyname, optarg); break;
            default:
    ...
    qlog_dbg("Press CTRL+C to stop %s\n", argv[0]);
    ...
    while (s_quit == 0)
    {
        ...
        for (n = 0; n < 2; n++)
        {
            ...
            if ((pollfds[n].revents & (POLLIN)) == 0) continue;
