...
typedef struct
{
    ...
    uint8 num_accel_rsp;
    diag_hw_accel_query_sub_payload_rsp_t sub_query_rsp[DIAG_HW_ACCEL_TYPE_MAX][DIAG_HW_ACCEL_VER_MAX];
...
static int diag_set_diag_transport(int peripheral_type, int peripheral, uint8 stm_cmd)
{
    ...
    switch (peripheral)
    {
        case DIAG_MODEM_PROC: req->processor_mask = DIAG_STM_MODEM; break;
        case DIAG_APPS_PROC: req->processor_mask = DIAG_STM_APPS; break;
...
static int diag_set_diag_qdss_diag_tracer(int peripheral_type, int peripheral, uint8 state)
{
    ...
    switch (peripheral)
    {
        case DIAG_APPS_PROC: req->subsys_cmd_code += DIAG_QDSS_PROCESSOR_APPS; break;
        case DIAG_MODEM_PROC: req->subsys_cmd_code += DIAG_QDSS_PROCESSOR_MODEM; break;
        default: return 0; break;
    }
...
int process_diag_hw_accel_query_rsp(int peripheral_type, const uint8_t *buf_ptr)
{
    ...
    else
    {
        ...
        // qlog_dbg("importment: trace_sink[0] = [%d]\n", rsp->query_rsp.diag_transport);
        for (i = DIAG_HW_ACCEL_TYPE_STM; i <= DIAG_HW_ACCEL_TYPE_MAX; i++)
        {
            ...
            qlog_dbg("hw_accel_support[%d][%d] = [%d]\n", peripheral_type, i, hw_accel_support[peripheral_type][i]);
        }
        return mask;
    }
...
/*int diag_id,int hw_accel_type, int hw_accel_ver, */
static int diag_send_enable_hw_accel_req(int peripheral_type, int peripheral, uint8 state)
{
    qlog_dbg("%s\n", __func__);
    (void)peripheral_type;
    (void)peripheral;
    ...
    unsigned char *ptr = qdss_cmd_req_buf;
    diag_hw_accel_cmd_req_t *req = (diag_hw_accel_cmd_req_t *)ptr;
    req->header.cmd_code = DIAG_SUBSYS_CMD_F;
    req->header.subsys_id = DIAG_SUBSYS_DIAG_SERV;
    req->header.subsys_cmd_code = DIAG_DIAG_HW_ACCEL_CMD;
    req->version = 1;
    req->operation = 1;
    req->op_req.hw_accel_type = 2;
    req->op_req.hw_accel_ver = 1;
    req->op_req.diagid_mask = 2;
    diag_send_data(ptr, diag_qsr4_append_crc16(ptr, sizeof(*req)));
...
int diag_send_cmds_to_peripheral_init(int peripheral_type, int pd) // 0,0
{
    uint8 sink;
    if (modem_is_pcie)
        sink = SINK_PCIE;
    ...
    for (retry = 0; retry < 10; retry++) // Repeat sending until sending succeeds(module to be ready)
    {
        ...
        if (hw_accel_support[peripheral_type][DIAG_HW_ACCEL_TYPE_ATB] || hw_accel_support[peripheral_type][DIAG_HW_ACCEL_TYPE_STM])
        {
        ...
        qlog_dbg("init: peripheral_init = [%d]\n", peripheral_init);
...
int diag_qdss_handle_response(const uint8_t *buf, size_t size)
{
    ...
    if (rsp->cmd_code == DIAG_BAD_CMD_F || rsp->cmd_code == DIAG_BAD_PARM_F)
    {
        ...
        if (hw_accel_query_state != 1) peripheral_init = 0;
    }

    // if (rsp->cmd_code == DIAG_SUBSYS_CMD_F && rsp->subsys_id == DIAG_SUBSYS_DIAG_SERV)
    if (rsp->cmd_code == DIAG_SUBSYS_CMD_F && (rsp->subsys_id == DIAG_SUBSYS_QDSS || rsp->subsys_id == DIAG_SUBSYS_DIAG_SERV))
    {
