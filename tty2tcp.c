    ...
    if (unused < size && fifo->size < CACHE_SIZE)
    {
        ...
        if (data)
        {
            ...
            if (fifo->data)
            {
                ...
                if (len) memcpy(data, fifo->data + fifo->out, len);
                free(fifo->data);
            }
...
#define FIFO_NUM 12
static struct __kfifo kfifo[FIFO_NUM] = {{-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL},
                                         {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}, {-1, 0, 0, 0, NULL}};
        ...
        if (tty2tcp_sockfd == -1 && tty2tcp_logfd == -1 && tty2tcp_thirdfd == -1)
        {
            ...
            if (g_is_qualcomm_chip)
            {
                ...
                if (use_diag_dpl)
                {
                    ...
                    if (tty2tcp_thirdfd == -1) break;
                }
            }
        }

        if (tty2tcp_sockfd != -1) pollfds[n++].fd = tty2tcp_sockfd;

        if (tty2tcp_logfd != -1) pollfds[n++].fd = tty2tcp_logfd;

        if (tty2tcp_thirdfd != -1) pollfds[n++].fd = tty2tcp_thirdfd;
...
static int tty2tcp_init_filter(int ttyfd, const char *cfg)
{
    ...
    tty2tcp_ttyfd = ttyfd;
    if (cfg) tty2tcp_tcpport = atoi(cfg);
