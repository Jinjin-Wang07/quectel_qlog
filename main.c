...
#define QLOG_VERSION "1.5.28_2" // when release, rename to V1.X
...
int g_module_from_ecm_2_rndis = 0;
int use_qmdl2_v2 = 0;
int use_diag_qdss = 0;
int use_diag_dpl = 0;
int disk_file_fd = -1;
...
int g_use_mapping_port = 0;
char g_use_mapping_path[256];
uint32_t g_ignore_capture_qdb_file = 0;
uint32_t g_kill_qlog = 0;
char g_ttyAT[256];
int g_send_at_flag = 0;
uint16_t g_use_qserver = 0;
struct QServer g_qserver;
char g_compress_cmd[256];
int g_use_compress_cmd = 0;
pthread_t g_compress_thread;
Queue *g_queue = NULL;
char g_compress_diag_file_name[512];
char g_compress_second_file_name[512];
char g_compress_third_file_name[512];
int g_unisoc_had_crash = 0;

#ifdef USE_NDK
const char *g_qlog_get_pid_path = "/data/qlog_pid.txt";
#else
const char *g_qlog_get_pid_path = "./qlog_pid.txt";
#endif
...
ssize_t qlog_poll_read_fds(int *fds, int n, void *pbuf, size_t size, unsigned timeout_msec)
{
    ...
    while (qlog_exit_requested == 0 && timeout_msec > 0)
    {
        ...
        for (i = 0; i < n; i++)
        {
            ...
            {
                // FIXME:
                // pbuf should not always begin from SEEK_SET when this function monitor
                // more than one fd (that is 'n > 0')
...
ssize_t qlog_poll_read(int fd, void *pbuf, size_t size, unsigned timeout_msec) { return qlog_poll_read_fds(&fd, 1, pbuf, size, timeout_msec); }
...
static int is_tftp(void) { return (g_tftp_server_ip != NULL); }

static int is_ftp(void) { return (g_ftp_server_ip != NULL); }
...
static int is_tcp_client(void) { return (g_tcp_client_port > 0); }

static int qlog_is_not_dir(const char *logdir) { return (is_ftp() || is_tftp() || !strncmp(logdir, "/dev/null", strlen("/dev/null"))); }
...
const char *qlog_time_name(int type)
{
    ...
    if (type == 1) // other
    {
        snprintf(time_name, sizeof(time_name), "%04d%02d%02d_%02d%02d%02d", (currtime->tm_year + 1900), (currtime->tm_mon + 1), currtime->tm_mday, currtime->tm_hour,
                 currtime->tm_min, currtime->tm_sec);
    }
    else if (type == 2) // for RM500K
    {
        snprintf(time_name, sizeof(time_name), "%04d_%02d%02d_%02d%02d%02d", (currtime->tm_year + 1900), (currtime->tm_mon + 1), currtime->tm_mday, currtime->tm_hour,
                 currtime->tm_min, currtime->tm_sec);
    }
    else if (type == 3) // for EC200U/EC600U
    {
        struct timespec ts;
        clock_gettime(CLOCK_MONOTONIC, &ts);
        snprintf(time_name, sizeof(time_name), "%02d-%02d-%02d-%02d-%03d", currtime->tm_mday, currtime->tm_hour, currtime->tm_min, currtime->tm_sec,
    ...
    else if (type == 4) // for 8850 EC800G
    {
        struct timespec ts;
        clock_gettime(CLOCK_MONOTONIC, &ts);
        snprintf(time_name, sizeof(time_name), "%04d_%02d_%02d_%02d_%02d_%02d_%03d", (currtime->tm_year + 1900), (currtime->tm_mon + 1), currtime->tm_mday, currtime->tm_hour,
...
{
    if (g_ftp_server_ip) return 0;

    char fullname[256];
    int fd = -1;
    size_t file_len = 0;

    if (qlog_args == NULL || qlog_args->logdir[0] == 0) return 0;
...
int qlog_create_file_in_logdir(const char *filename)
{
    ...
    if (qlog_args == NULL || qlog_args->logdir[0] == 0) return -1;
...
int unisoc_8910_ap_file_write_timestap(int logfd, struct unisoc_8910_aplog_timestamp *punisoc_ts)
{
    ...
    // snprintf(time_name, sizeof(time_name), "%02d-%02d-%02d-%02d-%03d",
    // currtime->tm_mday, currtime->tm_hour, currtime->tm_min, currtime->tm_sec,
    // (unsigned)(ts.tv_nsec / 1000000)); qlog_dbg("%s time_name:%s\n", __func__,
    // time_name);

    return qlog_logfile_save(logfd, punisoc_ts, sizeof(struct unisoc_8910_aplog_timestamp));
}

static int compare_logfile_mtime(const struct dirent **file1, const struct dirent **file2)
{
    ...
    /* 构建文件的完整路径 */
    snprintf(file1_path, sizeof(file1_path), "%s/%s", qlog_args->logdir, (*file1)->d_name);
    snprintf(file2_path, sizeof(file2_path), "%s/%s", qlog_args->logdir, (*file2)->d_name);

    stat(file1_path, &stat_file1);
    stat(file2_path, &stat_file2);
    return stat_file1.st_mtime - stat_file2.st_mtime;
}

static int filter_logfile(const struct dirent *entry)
{
...
static int remove_oldest_logfile(int total_size)
{
    struct stat file_stat;
    char last_file[1024];
    char last_zip_file[2048];
    int new_size = total_size;

    memset(last_file, 0, sizeof(last_file));
    memset(last_zip_file, 0, sizeof(last_zip_file));

    strcpy(last_file, peek(g_queue));

    snprintf(last_zip_file, sizeof(last_zip_file), "%s.zip", last_file);

    if (stat(last_zip_file, &file_stat) == 0)
    {
        if (remove(last_zip_file) == 0)
        {
            dequeue(g_queue);
            new_size -= file_stat.st_size;
            qlog_dbg("Deleted the oldest file: %s, total size: %d bytes\n", last_zip_file, new_size);
        }
        else
        {
            qlog_dbg("Error removing the oldest log file [%s]: %s\n", last_zip_file, strerror(errno));
        }
    }
    else if (stat(last_file, &file_stat) == 0)
    {
        /* 可能不是压缩文件，例如没来得及压缩 */
        if (remove(last_file) == 0)
        {
            dequeue(g_queue);
            new_size -= file_stat.st_size;
            qlog_dbg("Deleted the oldest file: %s, total size: %d bytes\n", last_file, new_size);
    ...
    {
        qlog_dbg("%s(.zip) stat error: %s\n", last_file, strerror(errno));
    }

    return new_size;
}

static int checkWorkDirSize(const char *dir_path)
{
    struct dirent *entry;
    ...
    char file_path[1024];
    int total_size = 0;
    int max_size = g_use_compress_cmd;
    int n;
    int fill_queue;
    char *zip_pos;

    qlog_dbg("[%s][%d] entry! [%s]\n", __func__, __LINE__, dir_path);

    /* g_queue为空说明是新启动，需要将文件加载到g_queue */
    fill_queue = is_empty(g_queue);

    /* 使用scandir按时间排序 */
    n = scandir(dir_path, &namelist, filter_logfile, compare_logfile_mtime);
    if (n < 0)
    {
        qlog_dbg("scandir error: %s\n", strerror(errno));
        return -1;
    }
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            qlog_dbg("stat error: %s\n", strerror(errno));
        }

        if (fill_queue)
        {
            /* 找到.zip位置，置\0后添加到queue */
            zip_pos = strstr(file_path, ".zip");
            if (zip_pos)
            {
                *zip_pos = '\0';
            }
            enqueue(g_queue, file_path);
        }
    ...
    qlog_dbg("Total size of files in directory '%s': %d bytes, max size: %d bytes\n", dir_path, total_size, max_size);

    // 删除最后修改时间最久的文件
    while ((total_size >= max_size) && !is_empty(g_queue))
    {
        total_size = remove_oldest_logfile(total_size);

        if (second_logfile_suffix && (is_empty(g_queue) == 0))
        {
            total_size = remove_oldest_logfile(total_size);
        }

        if (third_logfile_suffix && (is_empty(g_queue) == 0))
        {
            total_size = remove_oldest_logfile(total_size);
        }
    }
...
static int qlog_logfile_create(const char *logfile_dir, const char *logfile_suffix, unsigned logfile_seq)
{
    ...
    if (g_use_compress_cmd > 0)
    {
        checkWorkDirSize(logfile_dir);
    }
    ...
    qlog_dbg("[%s][%d] %s logfd=%d\n", __func__, __LINE__, filename, logfd);
    memset(g_compress_diag_file_name, '0', sizeof(g_compress_diag_file_name));
    strcpy(g_compress_diag_file_name, filename);
    if (g_use_compress_cmd > 0) enqueue(g_queue, filename);
    ...
    if (second_logfile_suffix)
    {
        ...
        qlog_dbg("[%s][%d] %s logfd=%d\n", __func__, __LINE__, filename, second_logfile);
        memset(g_compress_second_file_name, '0', sizeof(g_compress_second_file_name));
        strcpy(g_compress_second_file_name, filename);
        if (g_use_compress_cmd > 0) enqueue(g_queue, filename);
    ...
    if (third_logfile_suffix)
    {
        ...
        qlog_dbg("[%s][%d] %s logfd=%d\n", __func__, __LINE__, filename, third_logfile);
        memset(g_compress_third_file_name, '0', sizeof(g_compress_third_file_name));
        strcpy(g_compress_third_file_name, filename);
        if (g_use_compress_cmd > 0) enqueue(g_queue, filename);
    }
...
size_t qlog_logfile_save(int logfd, const void *buf, size_t size)
{
    ...
    if (idx != -1) return kfifo_write(idx, buf, size);

    return qlog_poll_write(logfd, buf, size, 1000);
}

static int qCompressLogFile(char *filename)
{
    qlog_dbg("[%s] entry file name [%s]\n", __func__, filename);
    char command[512];

    // 使用 snprintf 构造 compress 命令
    snprintf(command, sizeof(command), "./qcompress.sh %s ", filename);
    qlog_dbg("Do compress command: [%s]\n", command);
    ...
    if (result == 0)
    {
        qlog_dbg("File '%s' compressed successfully.\n", filename);
    }
    else
    {
        qlog_dbg("Error compressing file '%s'.\n", filename);
        return -1;
    }
...
static char compress_first_filename[512];
static char compress_second_filename[512];
static char compress_third_filename[512];

void *qCompressThread(void *arg)
{
    long type = (long)arg;
    qCompressLogFile(compress_first_filename);
    if (type > 1)
    {
        qCompressLogFile(compress_second_filename);
    }
    if (type > 3)
    {
        qCompressLogFile(compress_third_filename);
    }
    pthread_exit(NULL);
    return NULL;
}

static void updataComressFilename(long ret)
{
    if (ret)
    {
        memset(compress_first_filename, 0, sizeof(compress_first_filename));
        strcpy(compress_first_filename, g_compress_diag_file_name);
    }

    if (ret > 1)
    {
        memset(compress_second_filename, 0, sizeof(compress_second_filename));
        strcpy(compress_second_filename, g_compress_second_file_name);
    }

    if (ret > 3)
    {
        memset(compress_third_filename, 0, sizeof(compress_third_filename));
        strcpy(compress_third_filename, g_compress_third_file_name);
    }
...
{
    if ((!qlog_exit_requested) && (g_use_compress_cmd > 0))
    {
        long ret = 1;
        if (second_logfile > 0) ret += 2;
        if (third_logfile > 0) ret += 4;
        updataComressFilename(ret);
        pthread_create(&g_compress_thread, NULL, qCompressThread, (void *)ret);
    }
...
static void *qlog_logfile_init_filter_thread(void *arg)
{
    ...
    if (cfg && cfg->ops && cfg->ops->init_filter) cfg->ops->init_filter(cfg->fd, cfg->filter);
...
static int sendCfun2ATPort()
{
    int at_fd = serial_open(g_ttyAT);
    if (at_fd < 0) return -1;
    if (write(at_fd, "at+cfun=0\r\n", sizeof("at+cfun=0\r\n")) < 0) return -1;
    usleep(200);
    if (write(at_fd, "at+cfun=1\r\n", sizeof("at+cfun=1\r\n")) < 0) return -1;
    usleep(200);
    close(at_fd);
    qlog_dbg("[%s][%d] leave!\r\n", __func__, __LINE__);
    return 0;
...
static int message_handle_mdm(const struct arguments *args);
static int message_handle_asr(const struct arguments *args);
static int message_handle_unisoc(const struct arguments *args);
static int message_handle_eigen(const struct arguments *args);
static int message_handle_mtk(const struct arguments *args);

static int qlog_handle(const struct arguments *args)
{
    qlog_dbg("[%s] entry!\n", __func__);

    if (g_use_compress_cmd > 0 && (g_queue = create_queue()) == NULL)
    {
        qlog_dbg("[%s][%d] init queue error!\n", __func__, __LINE__);
        return -1;
    }

    if (g_is_asr_chip)
    {
        message_handle_asr(args);
    }
    else if (g_is_unisoc_chip || g_is_unisoc_exx00u_chip)
    {
        message_handle_unisoc(args);
    }
    else if (g_is_qualcomm_chip)
    {
        message_handle_mdm(args);
    }
    else if (g_is_mtk_chip)
    {
        message_handle_mtk(args);
    }
    else if (g_is_eigen_chip)
    {
        message_handle_eigen(args);
    }
    else
    {
        qlog_dbg("ERROR CHIP MODE,PLS CHECK!\n");
    }
...
{
    if (signal_num == SIGTERM || signal_num == SIGHUP || signal_num == SIGINT) qlog_exit_requested = 1;
...
{
    qlog_dbg("Usage: %s -p <log port> -s <log save dir> -f filter_cfg -n <log "
             "file max num> -b <log file size MBytes>\n",
             self);
    ...
    qlog_dbg("          if set as '9000', QLog will run in TCP Server Mode, and "
             "can be connected with 'QPST/QWinLog/CATStudio/Logel'!\n");
    qlog_dbg("          if set as 'IP:9000', QLog will run in TCP Client Mode, "
             "and send log to TCP Server, like 'nc -l 9000 > log.bin' \n");
    qlog_dbg("          if set as 'tftp:IP', QLog will run in TFTP Client Mode, "
             "and send log to TFTP Server, PC can run tftpd32/tftpd64 \n");
    qlog_dbg("          if set as 'ftp:IP-user:xxx-pass:xxx', The maximum value "
             "of user and pass is 32 bytes, QLog will run in FTP Client Mode, "
             "and send log to FTP Server, PC can run FileZilla Server \n");
    qlog_dbg("    -D    Delete all log files in the logdir before catching logs\n");
    qlog_dbg("          For instance: -D (indicate delete all supportted log), "
             "-Dqmdl (only delte log with suffix *.qmdl)\n");
    qlog_dbg("    -f    filter cfg for catch log, can be found in directory "
             "'conf'. if not set this arg, will use default filter conf\n");
    qlog_dbg("          and UC200T&EC200T do not need filter cfg.\n");
    qlog_dbg("    -n    max num of log file to save, range is '0~512'. default "
             "is 0. 0 means no limit.\n");
    qlog_dbg("          or QLog will auto delete oldtest log file if exceed max "
             "num\n");
    qlog_dbg("    -a    EXX00U capture blue screen dump, must to specify the "
             "gIsPanic address, like '-a 0x12345678', which can be obtained from "
             "the 8915dm_cat1.map file\n");
    qlog_dbg("    -m    max size of single log file, unit is MBytes, range is "
             "'2~512', default is 256\n");
    qlog_dbg("    -c    Determines whether to exit after QLog captures dump, "
             "default is 0, indicating exit\n");
    qlog_dbg("    -C    If you want to compress log file after catch log, you can use this option "
             "default is no-limit, but file number has 512 file number limit\n");
    qlog_dbg("    -g    For the state grid module, the module model needs to be "
             "specified, such as '-g EC200T'\n");
    qlog_dbg("    -i    EXX00U AP log is ignored, default is 0, indicating capture\n");
    qlog_dbg("    -j    Only capture qualcomm platform USB/PCIE module diag port "
             "logs, default is 0, indicating capture all ports logs\n");
    qlog_dbg("    -t    sony BG770A-GL captures uart logs, the -t parameter must "
             "be used and the COM port must be specified by -p, like './QLog -p "
             "/dev/ttyUSBCLI -p /dev/ttyUSBAUX -t'\n");
    qlog_dbg("          unisoc EXX00U captures uart ap logs, the -t parameter "
             "must be used , the COM port must be specified by -p and The -m "
             "parameter must be used to set the online ap log file to 10 MB, "
             "like './QLog -p /dev/ttyUSB0 -m 10 -t'\n");
    qlog_dbg("    -x    Capture log of udx710 NMEA port\n");
    qlog_dbg("    -q    Exit after usb disconnet\n");
    qlog_dbg("    -S    Use qlog_tcp_server mode, will save the remote tcp client log to local. eg:2500-2501-2502\n");

    qlog_dbg("\nNote: For the eigen platform module, the tool needs to be run "
             "within 16 seconds after the module dumps to successfully capture "
             "the dump, otherwise the timeout fails\n");
    qlog_dbg("\nFor example: %s -s .\n", self);
}
...
static void parser_tcp(const char *str)
{
    ...
    {
        if (strstr(str, ":") > strstr(str, "."))
            rc = sscanf(str, "%d.%d.%d.%d:%d", &ip[0], &ip[1], &ip[2], &ip[3], &g_tcp_client_port);
        else
            rc = sscanf(str, "%d:%d.%d.%d.%d", &g_tcp_client_port, &ip[0], &ip[1], &ip[2], &ip[3]);

        if (rc == 5)
        {
            snprintf(g_tcp_client_ip_tmp, sizeof(g_tcp_client_ip_tmp), "%d.%d.%d.%d", (uint8_t)ip[0], (uint8_t)ip[1], (uint8_t)ip[2], (uint8_t)ip[3]);
...
static struct arguments *parser_args(int argc, char **argv)
{
    ...
    while (-1 != (opt = getopt(argc, argv, "p:s:S:n:a:g:m:f:b:C:D::cjitqxhruke")))
    {
        if (opt == 'p' || opt == 's' || opt == 'n' || opt == 'a' || opt == 'g' || opt == 'm' || opt == 'f' || opt == 'b' || opt == 'C')
        {
            if (optarg == NULL)
            {
                goto error;
        ...
        {
            case 'u':
                g_use_mapping_port = 1;
                qlog_dbg("use mapping usb port: [%d]\r\n", g_use_mapping_port);
                break;
            case 'p':
                if (g_use_mapping_port)
                {
                    memset(g_use_mapping_path, 0, sizeof(g_use_mapping_path));
                    snprintf(g_use_mapping_path, sizeof(g_use_mapping_path), "%.255s", optarg);
                    qlog_dbg("g_use_mapping_path: [%s]\r\n", g_use_mapping_path);
                    ssize_t len = readlink(g_use_mapping_path, args.ttyDM, sizeof(args.ttyDM) - 1);
                    if (len != -1)
                    {
                        args.ttyDM[len] = '\0';
                        qlog_dbg("Symbolic link target: [%s]-[%s]\n", g_use_mapping_path, args.ttyDM);
                        break;
                    }
                    else
                    {
                        qlog_dbg("no link auto find");
                    }
                }
                if (args.ttyDM[0] == '\0')
                {
            ...
            case 'S':
                g_use_qserver = 1;
                g_qserver.dm_server_port = 0;
                g_qserver.qdss_server_port = 0;
                g_qserver.adpl_server_port = 0;
                g_qserver.file_path = NULL;
                g_qserver.file_num = 0;
                g_qserver.file_size = 0;
                parserServerPort(optarg);
                break;
            case 'D': args.delete_logs = optarg ? optarg : ""; break;
            case 'n':
            ...
            case 't':
                ...
                break;
            case 'f': args.filter_cfg = optarg; break;
            case 'q': exit_after_usb_disconnet = 1; break;
            case 'r': {
                g_module_from_ecm_2_rndis = 1;
                qlog_dbg("will in rndis mode: %d\n", g_module_from_ecm_2_rndis);
                break;
            }
            case 'x':
                ...
                break;
            case 'e':
                g_ignore_capture_qdb_file = 1;
                qlog_dbg("set ignore qdb file: %d\n", g_ignore_capture_qdb_file);
                break;
            case 'k':
                g_kill_qlog = 1;
                qlog_dbg("kill qlog: %d\n", g_kill_qlog);
                break;
            case 'b':
                memset(g_ttyAT, 0, sizeof(g_ttyAT));
                snprintf(g_ttyAT, sizeof(g_ttyAT), "%.255s", optarg);
                g_send_at_flag = 1;
                qlog_dbg("the at path:[%d] [%s]\n", g_send_at_flag, g_ttyAT);
                break;
            case 'C':
                g_use_compress_cmd = atoi(optarg) * 1024 * 1024;
                qlog_dbg("compress cmd:[%d]\n", g_use_compress_cmd / (1024 * 1024));
                break;
            case 'h':
            default: qlog_usage(argv[0], "/dev/ttyUSB0"); goto error;
        }
    }
...
{
    if ((idVendor == 0x2c7c && ((idProduct & 0xF000) == 0x6000)) && idProduct != 0x6007 && idProduct != 0x6008 && idProduct != 0x6009) // ASR  0x6007 is eigen; 0x6008 and 0x6009
                                                                                                                                       // both is SDX12-EM061KGL
        return 1;

    if (!strncasecmp(modem_name_para, "EC200T", 6) // GW EC200T
        && ((idVendor == 0x3763 && idProduct == 0x3c93) || (idVendor == 0x3c93 && idProduct == 0xffff)))
        return 1;
...
int drv_is_unisoc(int idProduct, int idVendor)
{
    if (idVendor == 0x2c7c && idProduct == 0x0900)
        ...
        return 1;
    else if (!strncasecmp(modem_name_para, "EC200D",
                          6) // GW EC200D     2 capture log    3 capture apdump
             && ((idVendor == 0x3763 && idProduct == 0x3c93) || (idVendor == 0x3c93 && idProduct == 0xffff)))
        return 2;
...
int drv_is_mtk(int idProduct, int idVendor)
{
    if (idVendor == 0x2c7c && idProduct == 0x7001)
        ...
        return 1;
    else if (idVendor == 0x2c7c && idProduct == 0x7006)
        return 1;
    else if (idVendor == 0x2c7c && idProduct == 0x7003)
        return 1;
    else if (idVendor == 0x2c7c && idProduct == 0x7008)
        return 1;
...
static int option_send_setup(int usbfd, int interface_num, int dtr_state, int rts_state)
{
    ...
    if (dtr_state) val |= 0x01;
    if (rts_state) val |= 0x02;
...
static int prepare(struct arguments *args)
{
    ...
    use_qmdl2_v2 = 0;
    if (usb_dev->idProduct == 0x0800 || usb_dev->idProduct == 0x0455 || usb_dev->idProduct == 0x0801 || usb_dev->idProduct == 0x0457 || usb_dev->idProduct == 0x0458 ||
        usb_dev->idProduct == 0x0452)
    {
        use_qmdl2_v2 = 1;
    }
    ...
    { // cp
        ...
        if (g_is_unisoc_exx00u_chip || g_is_unisoc_chip == 2 || g_is_unisoc_chip == 4) option_send_setup(cfg.usbfd, usb_dev->dm_intf.bInterfaceNumber, 1,
                                                                                                         1); // cp bInterfaceNumber
    }
    ...
    { // ap
        ...
        if (g_is_unisoc_exx00u_chip || g_is_unisoc_chip == 2 || g_is_unisoc_chip == 4)
            option_send_setup(cfg.usbfd, usb_dev->general_intf.bInterfaceNumber, 1,
                              1); // cp bInterfaceNumber
    }
...
static void delete_logs(const char *dir, const char *suffix)
{
    ...
    {
        if (entptr->d_name[0] == '.') continue;

        if (g_use_compress_cmd > 0)
        {
            snprintf(file_path, sizeof(file_path), "%s/%s", dir, entptr->d_name);
        ...
        if (!str_has_suffix(entptr->d_name, ".sdl") && !str_has_suffix(entptr->d_name, ".qmdl") && !str_has_suffix(entptr->d_name, ".qmdl2") &&
...
{
    qlog_dbg("[%s] [%d] entry\n", __func__, __LINE__);
    int fd = open(g_qlog_get_pid_path, O_WRONLY | O_CREAT | O_TRUNC, 0644);
    if (fd < 0)
    {
        perror("file");
        return -1;
    ...
    int num = getpid();
    ssize_t written = write(fd, &num, sizeof(num));
    if (written < 0)
    {
        perror("write");
        close(fd);
    ...
    qlog_dbg("Written %ld bytes\n", written);
    close(fd);

    return 0;
}

// stop_qlog is called when another instance of qlog is to be killed off
static int stop_qlog(void)
{
    qlog_dbg("[%s] [%d] entry\n", __func__, __LINE__);
    int fd = open(g_qlog_get_pid_path, O_RDONLY);
    if (fd < 0)
    {
        perror("file");
        return -1;
    ...
    {
        qlog_dbg("No data to read\n");
        close(fd);
        return -1;
    }

    qlog_dbg("Read integer: %d\n", num);
    close(fd);

    if (num <= 0 || (kill(num, SIGTERM)) < 0)
    {
        qlog_dbg("qlog: Unable to kill qlog instance pid: %d, errno: %d\n", num, errno);
    }
    else
    {
        qlog_dbg("qlog: stopping qlog instance pid: %d\n", num);
    }

    return 0;
}

static int catchUnisocETBData(const char *path, const char *diag_port);
int main(int argc, char **argv)
{
    ...
    if (g_use_qserver)
    {
        g_qserver.file_path = args->logdir;
        g_qserver.file_num = args->logfile_num;
        g_qserver.file_size = args->logfile_sz;
        qSocketServerHandle();
        return 0;
    }
    ...
    {
        qlog_com_catch_log(args->ttyDM, args->ttyGENERAL, args->logdir, args->logfile_sz,
                           qlog_time_name); // COM port data can only be stored locally
        return 0;
    }

    if (args->delete_logs) delete_logs(args->logdir, args->delete_logs);

__restart:
    qlog_dbg("[%d] restart entry\r\n", __LINE__);
    if (qlog_exit_requested) return 0;

    if (g_use_mapping_port > 1 && g_use_mapping_path[0] != '0')
    {
        ssize_t len = readlink(g_use_mapping_path, args->ttyDM, sizeof(args->ttyDM) - 1);
        if (len != -1)
        {
            args->ttyDM[len] = '\0';
            qlog_dbg("restart: symbolic link target: [%s]-[%s]\n", g_use_mapping_path, args->ttyDM);
        }
        else
        {
            qlog_dbg("restart: canot link [%s]-[%s]\n", g_use_mapping_path, args->ttyDM);
        }
    }
    ...
    else if (strStartsWith(args->ttyDM, "/dev/wwan"))
    {
        ...
        if (strstr(args->ttyDM, "qcdm") || strstr(args->ttyDM, "QCDM")) wwan_dev->bNumInterfaces = 5;
...
#endif
            qlog_dbg("No Quectel Modules found, Wait for connect or Press CTRL+C to "
                     "quit!\n");
            sleep(2);
            goto __restart;
        }
    ...
    if (strStartsWith(args->ttyDM, "/dev/ttyUSB") || strStartsWith(args->ttyDM, "/dev/ttyACM") || strStartsWith(args->ttyDM, "/sys/bus/usb/"))
    {
        ...
        {
            if (!strcmp(args->ttyDM, s_usb_device_info[cur_module].usbdevice_pah)) break;

            if (!strcmp(args->ttyDM, s_usb_device_info[cur_module].ttyDM) || !ql_match_dm_device(args->ttyDM, &s_usb_device_info[cur_module]))
            {
                break;
            }
        }
    ...
    if (s_usb_device_info[cur_module].idProduct == 0x4d00 && s_usb_device_info[cur_module].idVendor == 0x1782)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (access(devpath, F_OK)) goto __restart;
            sleep(1);
        }
    }
    ...
    if (args->ql_dev->idProduct == 0x4d00 && args->ql_dev->idVendor == 0x1782 && args->ql_dev->bcdDevice == 1) g_is_unisoc_chip = 3; // EC200D ap dump
    ...
    loop_times++;
    if (access(args->logdir, F_OK) && errno == ENOENT && !is_tftp() && !is_ftp() && !is_tty2tcp() && !is_tcp_client()) mkdir(args->logdir, 0755);
    ...
    if (args->ql_dev->bNumInterfaces == 1)
    {
        ...
        if (qlog_is_not_dir(args->logdir))
        {
            ...
            int n = snprintf(dump_dir, sizeof(dump_dir), "%s", args->logdir);

            if (n < 0 || (size_t)n >= sizeof(dump_dir)) {
                fprintf(stderr, "logdir path too long\n");
                exit(EXIT_FAILURE);
            }
        }
        else if (is_tty2tcp())
    ...
    else if (args->ql_dev->bNumInterfaces > 1)
    {
        ...
        if (is_tftp())
        {
            qlog_dbg("tftp only support catch ram dump, but modem is not in ram dump "
                     "state\n");
            qlog_exit_requested = 1;
            goto error;
        }
...
error:
    ...
    if (g_unisoc_had_crash)
    {
        g_unisoc_had_crash = 0;
        sleep(5);
        catchUnisocETBData(args->logdir, args->ql_dev->ttyDM);
    }
...
static int message_handle_mdm(const struct arguments *args)
{
    ...
    if (args->ql_dev->general_type == MDM_QDSS) second_logfile_suffix = "_qdss.bin";

    third_logfile_suffix = NULL;
    if (args->ql_dev->third_type == 1) third_logfile_suffix = ".adplv";
    ...
    if (!qlog_ops.logfile_save) qlog_ops.logfile_save = qlog_logfile_save;
    if (!qlog_ops.logfile_close) qlog_ops.logfile_close = qlog_logfile_close;
    ...
    if (g_send_at_flag)
    {
        if (sendCfun2ATPort() == -1)
        {
            qlog_dbg("send cfun error\r\n");
        }
        g_send_at_flag = 0;
    }
    ...
    while (!qlog_exit_requested)
    {
        ...
        rc = qlog_poll_read_fds(fds, fd_n, rbuf, rbuf_size, 30000);
        if (rc <= 0)
        {
            if (qlog_exit_requested == 0)
            {
        ...
        if (fds[0] == dmfd)
        {
            ...
            wc = qlog_ops.logfile_save(logfile_fd, rbuf, rc);
            savelog_size_dm += wc;
        ...
        else if (fds[0] == generalfd)
        {
            ...
            {
                g_qualcomm_log_type = 1;
                wc = qlog_ops.logfile_save(second_logfile, rbuf, rc);
                savelog_size_general += wc;
                savelog_size_general_all += wc;
            }
            else if (second_logfile_suffix)
            {
                wc = qlog_logfile_save(second_logfile, rbuf, rc);
                savelog_size_general += wc;
                savelog_size_general_all += wc;
            }
            else
            {
                qlog_dbg("save mdm log error!\n");
            }
        }
        else if (fds[0] == thirdfd)
        {
            ...
            {
                g_qualcomm_log_type = 2;
                wc = qlog_ops.logfile_save(third_logfile, rbuf, rc);
            ...
            else if (third_logfile_suffix)
            {
                ...
                {
                    wc = qlog_logfile_save(third_logfile, rbuf, rc);
                }
                savelog_size_third += wc;
                savelog_size_third_all += wc;
            }
        ...
        {
            qlog_dbg("recv time:[%d]\n", now_msec);
            qlog_dbg("recv qmdl:[%zdM][%zdK][%zdB] \n", savelog_size_dm_all / (1 * 1024 * 1024), savelog_size_dm_all / 1024 % 1024, savelog_size_dm_all % 1024);
            if (generalfd > 0)
            {
                qlog_dbg("recv qdss:[%zdM][%zdK][%zdB] \n", savelog_size_general_all / (1 * 1024 * 1024), savelog_size_general_all / 1024 % 1024, savelog_size_general_all % 1024);
            }
            if (thirdfd > 0)
            {
                qlog_dbg("recv adpl:[%zdM][%zdK][%zdB] \n", savelog_size_third_all / (1 * 1024 * 1024), savelog_size_third_all / 1024 % 1024, savelog_size_third_all % 1024);
            }
            last_msec = now_msec;
        }
    ...
    if (logfile_fd != -1)
    {
        ...
        if (savelog_size_dm_all != 0) qlog_dbg("savelog_size_dm_all = %ld\n", savelog_size_dm_all);

        if (savelog_size_general_all != 0) qlog_dbg("savelog_size_general_all = %ld\n", savelog_size_general_all);

        if (savelog_size_third_all != 0) qlog_dbg("savelog_size_third_all = %ld\n", savelog_size_third_all);
    }
...
static int message_handle_asr(const struct arguments *args)
{
    ...
    while (!qlog_exit_requested)
    {
        ...
        if (now_msec >= (last_msec + 5000))
        {
            qlog_dbg("recv:time[%d] sdl:[%zdM] [%zdK] [%zdB] \n", now_msec, savelog_size_dm_all / (1 * 1024 * 1024), savelog_size_dm_all / 1024 % 1024, savelog_size_dm_all % 1024);

            last_msec = now_msec;
        }

        savelog_size_total = savelog_size_dm;
        if (savelog_size_total >= logfile_size && g_donot_split_logfile == 0)
        {
            savelog_size_dm = 0;
            savelog_size_total = 0;
...
static int message_handle_unisoc(const struct arguments *args)
{
    ...
    second_logfile_suffix = NULL;
    if (args->ql_dev->general_type == EC200U_AP) second_logfile_suffix = ".bin";

    third_logfile_suffix = NULL;
    if (args->ql_dev->third_type == 2) third_logfile_suffix = ".log";
    ...
    while (!qlog_exit_requested)
    {
        ...
        if (g_is_unisoc_chip == 2) // unisoc EC200D
        {
            if (rc >= 10) // sizeof(apdump_recv_buf)
            {
                ...
                {
                    qlog_dbg("EC200D-CN into ap dump...\n");
                    unisoc_catch_8310_dump(dmfd, logfile_dir, RX_URB_SIZE, qlog_time_name);
                    sleep(1);
                    qlog_exit_requested = 1; // Unisoc 8310 captures AP dumps and requires
                                             // the exit flag to be preserved
                    break;
                }
            }
        }
        ...
        if (g_is_unisoc_exx00u_chip == 1 && rbuf[0] == 0xad && rbuf[1] == 0x00 && rbuf[2] == 0x03 && rbuf[3] == 0xff && rbuf[5] == 0x01) // check whether it is blue screen dump
        {
            qlog_ops_t *tmp_unisco_ops = &qlog_ops;
            int tmp_ap_fd = second_logfile;
            int ret_blue_screen = unisoc_exx00u_catch_blue_screen(rbuf, logfile_dir, tmp_unisco_ops, tmp_ap_fd);
        ...
        {
            qlog_dbg("recv time:[%d]\n", now_msec);
            qlog_dbg("recv DM:[%zdM][%zdK][%zdB] \n", savelog_size_dm_all / (1 * 1024 * 1024), savelog_size_dm_all / 1024 % 1024, savelog_size_dm_all % 1024);
            if (generalfd > 0)
            {
                qlog_dbg("recv BIN:[%zdM][%zdK][%zdB] \n", savelog_size_general_all / (1 * 1024 * 1024), savelog_size_general_all / 1024 % 1024, savelog_size_general_all % 1024);
            }
            if (thirdfd > 0)
            {
                qlog_dbg("recv LOG:[%zdM][%zdK][%zdB] \n", savelog_size_third_all / (1 * 1024 * 1024), savelog_size_third_all / 1024 % 1024, savelog_size_third_all % 1024);
            }
            last_msec = now_msec;
        }
        ...
        if (g_is_unisoc_exx00u_chip)
        {
            if (savelog_size_general > (AP_LOG_SIZE - rbuf_size) || savelog_size_dm >= logfile_size)
            {
                savelog_size_dm = 0;
                savelog_size_general = 0;
                qlog_ops.logfile_close(logfile_fd);
    ...
    if (g_use_compress_cmd > 0) destroy_queue(g_queue);
...
static int message_handle_eigen(const struct arguments *args)
{
    ...
    while (!qlog_exit_requested)
    {
        ...
        if (now_msec >= (last_msec + 5000))
        {
            qlog_dbg("recv:time[%d] BIN:[%zdM] [%zdK] [%zdB] \n", now_msec, savelog_size_dm_all / (1 * 1024 * 1024), savelog_size_dm_all / 1024 % 1024, savelog_size_dm_all % 1024);
...
static int message_handle_mtk(const struct arguments *args)
{
    ...
    uint8_t *rbuf;
    const size_t rbuf_size = QLOG_BUF_SIZE;
    const char *logfile_suffix = "muxraw";
    static qlog_ops_t qlog_ops;
    pthread_t init_filter_tid;
    init_filter_cfg_t init_filter_cfg;
    unsigned now_msec = 0;
    ...
    while (!qlog_exit_requested)
    {
        ...
        if (now_msec >= (last_msec + 5000))
        {
            qlog_dbg("recv:time[%d] muxraw:[%zdM] [%zdK] [%zdB] \n", now_msec, savelog_size_dm_all / (1 * 1024 * 1024), savelog_size_dm_all / 1024 % 1024,
                     savelog_size_dm_all % 1024);
            last_msec = now_msec;
        }
...
void trim_file(const char *path, const char *input_file, size_t n, size_t m);
#define TIMEOUT_SEC 10
static int catchUnisocETBData(const char *path, const char *diag_port)
{
    qlog_dbg("[%s][%d] entry! port:[%s]\n", __func__, __LINE__, diag_port);
    char etb_file_path[512];
    int usb_fd = serial_open(diag_port);
    if (usb_fd == -1)
    {
        qlog_dbg(" error:open [%s]\n", strerror(errno));
        return -1;
    }

    int val = fcntl(usb_fd, F_GETFL, 0);
    qlog_dbg("post-open file status = 0x%x\n", val);

    fcntl(usb_fd, F_SETFL, 0); /* suspect line */

    val = fcntl(usb_fd, F_GETFL, 0);
    qlog_dbg("post-fcntl file status = 0x%x\n", val);

    memset(etb_file_path, 0, sizeof(etb_file_path));
    strcat(etb_file_path, path);
    strcat(etb_file_path, "/tmc_etb.bin");
    int file_fd = qlog_logfile_create_fullname(0, etb_file_path, 0, 1);
    if (file_fd < 0)
    {
        qlog_dbg("etb file create error! file path: [%s]\n", etb_file_path);
        return -1;
    }

    // Define the hex data buffer to send
    unsigned char hex_data[] = {0x7E, 0x00, 0x00, 0x00, 0x00, 0x0C, 0x00, 0x62, 0x00, 0x33, 0x00, 0x00, 0x00, 0x7E}; // Example hex data
    size_t hex_data_len = sizeof(hex_data);

    // Send the hex buffer to USB
    if (write(usb_fd, hex_data, hex_data_len) < 0)
    {
        qlog_dbg(" error:write [%s]\n", strerror(errno));
        close(usb_fd);
        close(file_fd);
        return -1;
    }

    // Buffer to receive data from USB
    ...
    while (1)
    {
        ...
        {
            qlog_dbg("Timeout occurred! No data received for %d seconds.\n", TIMEOUT_SEC);
            break; // 超时，退出
        }
        else if (FD_ISSET(usb_fd, &readfds))
        {
            bytes_read = read(usb_fd, recv_buffer, sizeof(recv_buffer) - 1);
            qlog_dbg("have data [%d]\n", bytes_read);
            if (bytes_read <= 0)
            {
                perror("read");
                break;
            ...
            int ret = write(file_fd, recv_buffer, bytes_read);
            ...
            if (ret != bytes_read)
            {
                qlog_dbg(" error:write file W:[%d] R:[%d] [%s]\n", ret, bytes_read, strerror(errno));
            }
        }
    }
    ...
    trim_file(path, etb_file_path, 35, 19);
...
void trim_file(const char *path, const char *input_file, size_t n, size_t m)
{
    ...
    if (in == NULL)
    {
        qlog_dbg("can not open the file: [%s]\n", strerror(errno));
        return;
    }
    ...
    // 计算新的文件大小
    long new_size = file_size - n - m;
    if (new_size < 0)
    {
        qlog_dbg("error: file size < n+m, [%s]\n", strerror(errno));
        fclose(in);
        return;
    }
    ...
    // 创建临时输出文件
    char file_peth[512];
    memset(file_peth, 0, sizeof(file_peth));
    strcat(file_peth, path);
    strcat(file_peth, "/");
    strcat(file_peth, qlog_time_name(1));
    strcat(file_peth, "_etb.bin");
    FILE *out = fopen(file_peth, "wb");
    if (out == NULL)
    {
        qlog_dbg("canot open the out file %s\n", file_peth);
        fclose(in);
        return;
    }
    ...
    // 读取并写入剩余的数据
    char buffer[1024];
    size_t bytes_to_read = new_size;
    while (bytes_to_read > 0)
    {
        size_t read_size = (bytes_to_read < sizeof(buffer)) ? bytes_to_read : sizeof(buffer);
        size_t read_count = fread(buffer, 1, read_size, in);
        fwrite(buffer, 1, read_count, out);
    ...
    {
        qlog_dbg("can not remove the real file [%s]\n", strerror(errno));
        return;
    }

    qlog_dbg("successful remove out put file name: %s\n", file_peth);
    return;
}