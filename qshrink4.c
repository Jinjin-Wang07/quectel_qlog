...
typedef struct
{
    ...
    uint32_t guid_struct[MAX_GUID_ENTRIES][1 + 30 + GUID_LEN]; // reserver for diagid_guid_struct
...
static const char *guid_file_name(const uint8_t *guid)
{
    ...
    snprintf(read_buf, 100, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x.qdb", qlog_le32(guid_val->data1), qlog_le16(guid_val->data2), qlog_le16(guid_val->data3),
...
static int diag_qsr4_get_cmd_code_for_peripheral(int peripheral)
{
    switch (peripheral)
    {
        case DIAG_MODEM_PROC: return DIAGDIAG_QSR4_FILE_OP_MODEM;
        default: return -1;
    }
}
...
{
    if (!process_name) return -EINVAL;

    if (strstr(process_name, search_str)) return 1;
...
static int diag_query_pd(const char *process_name)
{
    ...
    if (diag_query_pd_name(process_name, "APPS")) return DIAG_APPS_PROC;
    if (diag_query_pd_name(process_name, "Apps")) return DIAG_APPS_PROC;
    if (diag_query_pd_name(process_name, "modem/root_pd")) return DIAG_MODEM_PROC;
...
static diag_id_list *get_diag_id(int peripheral)
{
    ...
    for (i = 0; i < diag_id_count; i++)
    {
        ...
        if ((peripheral == item->peripheral) && diag_query_pd_name(item->process_name, "root")) return item;
    }
...
static void insert_diag_qsr4_db_guid_to_list(qshrink4_file_info *db_file_info, int peripheral)
{
    ...
    if (guid_list_entry_count >= MAX_GUID_ENTRIES) return;
...
static int diag_send_qsr4_db_file_list_cmd_req(int peripheral)
{
    ...
    if (diag_qsr4_get_cmd_code_for_peripheral(peripheral) == -1) return 0;
...
static int diag_send_qsr4_file_read_cmd_req(unsigned int file_offset, int file_len)
{
    ...
    if (read_file_fd == -1) return 0;

    if (file_offset >= read_file_total_len) return 0;
    ...
    if (diag_verbose || (file_offset == 0 || ((file_offset + file_len) == read_file_total_len))) qlog_dbg("%s offset=%08d, len=%d\n", __func__, file_offset, file_len);
...
static void insert_diag_id_entry(diag_id_entry_struct *entry)
{
    ...
    if (diag_id_count >= NUM_PROC) return;
...
static void mdm_handle_diag_pkt_func(struct diag_pkt *diag_pkt)
{
    if (g_mdm_req != -1)
    {
        ...
        if ((diag_pkt->buf[0] == g_mdm_req) || ((diag_pkt->buf[0] == DIAG_BAD_PARM_F && diag_pkt->buf[1] == g_mdm_req)) ||
...
static size_t ftp_qdb_file_len = 0;
extern int g_ignore_capture_qdb_file;
static void mdm_create_qshrink4_file(int fd)
{
    // static int qshrink4_init = 0;

    if (qshrink4_init) return;
    qshrink4_init = 1;
    ...
    if (use_qmdl2_v2 && diag_id_count == 0)
    {
        ...
        for (i = 0; i < qlog_le32(qshrink4_data.guid_list_entry_count); i++)
        {
            ...
            if (guid_file_len == qlog_le32(qshrink4_data.guid_file_len[i])) continue;

            if (ftp_qdb_file_len != 0) // for ftp qdb
            {
                qlog_dbg("qdb file already exist\n");
                continue;
            }

            if (g_ignore_capture_qdb_file)
            {
                continue; // Never catch qdb under any circumstances
            }
            ...
            ftp_qdb_file_len = qlog_le32(qshrink4_data.guid_file_len[i]);
        }
