...
extern int g_module_from_ecm_2_rndis;
...
#define QLOG_BUF_SIZE \
    (64 * 1024) // TTYB_DEFAULT_MEM_LIMIT 65536 -> (640 * 1024UL)
...
extern ssize_t qlog_poll_read_fds(int *fds, int n, void *pbuf, size_t size, unsigned timeout_msec);
...
extern int qlog_com_catch_log(char *ttyDM, char *ttyGENERAL, char *logdir, int logfile_sz, const char *(*qlog_time_name)(int));
extern int unisoc_exx00u_catch_blue_screen(uint8_t *pbuf, const char *logfile_dir, const qlog_ops_t *tmp_unisco_ops, const int tmp_ap_fd);
