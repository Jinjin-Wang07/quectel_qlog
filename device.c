...
#define file_get_str(path)                            \
    ({                                                \
        const char *line = file_get_first_line(path); \
        SAFETYSTR(line);                              \
    })
#define file_get_int(path, invalid)                   \
    ({                                                \
        const char *line = file_get_first_line(path); \
        line ? atoi(line) : invalid;                  \
    })
#define file_get_xint(path, invalid)                  \
    ({                                                \
        const char *line = file_get_first_line(path); \
        line ? strtoul(line, NULL, 16) : invalid;     \
    })
...
static uint8_t usb_intf_get_endpoint(const char *dirname, enEndpointType eptype)
{
    ...
    while ((entptr = readdir(dirptr)))
    {
        ...
        if (strncasecmp(entptr->d_name, "ep_", 3)) continue;
