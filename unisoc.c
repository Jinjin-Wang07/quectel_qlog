...
static struct ql_unisoc_exx00u_xml_data ql_exx00u_xml_data_defult = {{{{{"region", "bootrom", 0x00000000, 0x10000, 0}}},
...
static struct ql_unisoc_ec800g_xml_data ql_ec800g_xml_data_defult = {{{{{"region", "ap_iram", 0x00100000, 0x4000, 0}}},
...
{
    if (frm_chk((unsigned short *)pHdr, SMP_HDR_LEN, 0) == 0) return 1;
...
static int unisoc_init_filter(int fd, const char *cfg)
{
    ...
    while (qlog_exit_requested == 0)
    {
        if (!m_bSendTCmd && m_bUE_Obtained == 0 && nTryNum++ < 5)
        {
            ...
            if (qlog_poll_write(fd, lpBuf, DIAG_HDR_LEN + 2, 0) <= 0)
            {
                return -1;
            }
        }
        ...
        */
        ...
        if (!m_bSendTCmd)
        {
            ...
            if (write(fd, asser_modem, sizeof(asser_modem)) == -1)
            {
                return -1;
            };
        }
...
static void unisoc_proc_log_buf(int logfd, const void *inBuf, size_t size)
{
    ...
    while (cur < size)
    {
        ...
        if (memcmp(lpBuf, agStartFlag, SMP_START_LEN))
        {
            if (cur == 0)
            {
                // qlog_dbg("%s agStartFlag %02x%02x%02x%02x\n", __func__, lpBuf[0], lpBuf[1],
                // lpBuf[2], lpBuf[3]); hexdump(lpBuf, MIN(100, size-cur));
            }
            cur++;
            if (cur == size)
            {
        ...
        {
            qlog_dbg("%s header.packet_type=0x%x\n", __func__, lpPkg->header.packet_type);
            hexdump(lpBuf, MIN(100, size - cur));
            cur++;
            continue;
        }
        ...
        {
            // qlog_dbg("%s wait for more data, cur=%zd, size=%zd, len=%d\n", __func__, cur, size,
            // lpPkg->header.len);
            break;
        }
    ...
    if (unisoc_log_size) memmove(unisoc_log_buf, lpBuf, unisoc_log_size);
...
static int HandleDiagPkt(const uint8_t *lpBuf, size_t size)
{
    ...
    if (pkt_len != (size - 2))
    {
        if (pkt_len) qlog_dbg("fix pkt_len= %d -> %zd\n", pkt_len, size - 2);
        pkt_len = size - 2;
...
error:
    if (pbuf_dump) free(pbuf_dump);
    if (unisoc_logfile_dump != -1) qlog_logfile_close(unisoc_logfile_dump);
...
uint8_t PacketCrc(const void *packet, int size)
{
    ...
    uint8_t crc = 0;
    for (i = 3; i < size - 1; i++) crc ^= p[i];

    return crc;
}

size_t unisoc_exx00u_send_cmd(int fd, uint8_t *buff, size_t size) { return qlog_poll_write(fd, buff, size, 1000); }

size_t unisoc_exx00u_recv_cmd(int fd, uint8_t *buff, size_t size) { return qlog_poll_read(fd, buff, size, 3000); }
...
size_t unisoc_exx00u_send_cmd_complete(int fd, uint32_t addr, uint16_t size, int type)
{
    ...
    switch (type)
    {
        ...
        default: qlog_dbg("unisoc_exx00u_cmd_complete switch error\n");
    }
...
size_t checkReadBlock(const uint8_t *buff, const size_t rc, uint8_t *save_buff)
{
    ...
    data_size = (buff[1] << 8) | buff[2];

    if ((data_size + 4) > rc)
    {
        // qlog_dbg("read buff length error rc:[%ld] size:[%d]\r\n", rc, data_size);
        return 0;
    }

    // data_crc = buff[3 + data_size];

    // if (data_crc != PacketCrc(&buff[start_index], data_size + 3))
    // {
    //     qlog_dbg("read buff crc error crc:[%d] buff crc:[%d]\r\n", data_crc, PacketCrc(buff + start_index, data_size + 3));
    //     return -1;
    // }
    ...
    if (data_rid != rid)
    {
        // qlog_dbg("read buff rid check error rid:[%x] buff rid:[%x] [%x]\r\n", rid, data_rid, data_crc);
        return 0;
    }

    memcpy(save_buff, buff + 5, data_size - 2);
    return data_size - 2;
}

size_t checkRead(const uint8_t *buff, const size_t rc, uint32_t *pvalue)
{
    // uint8_t data_crc = 0;
    uint8_t data_rid = -1;
    ...
    // if (data_crc != PacketCrc(&buff[start_index], 9))
    // {
    //     qlog_dbg("read buff crc error crc:[%d] buff crc:[%d]\r\n", data_crc, PacketCrc(buff + start_index, data_size + 3));
    //     return -1;
    // }
    ...
    *pvalue = (buff[5]) << 24 | (buff[6]) << 16 | (buff[7]) << 8 | buff[8];
    return rc;
}

size_t unisoc_exx00u_recv_cmd_deal(int fd, uint8_t *pbuf, uint32_t *pvalue, int type, uint8_t *save_buff, const qlog_ops_t *tmp_unisco_ops, const int tmp_ap_fd)
{
    size_t rc = 0;
    do
    {
        rc = unisoc_exx00u_recv_cmd(fd, pbuf, QLOG_BUF_SIZE);
        if (!(pbuf[0] == 0xad && pbuf[3] == 0xfe))
        {
            tmp_unisco_ops->logfile_save(tmp_ap_fd, pbuf, rc);
            rc = 0;
        }
    ...
    {
        case 0: {
            rc = checkReadBlock(pbuf, rc, save_buff);
            break;
        }
        case 1: {
            checkRead(pbuf, rc, pvalue);
            break;
        }
        default: break;
    }
...
int unisoc_exx00u_catch_blue_screen(uint8_t *pbuf, const char *logfile_dir, const qlog_ops_t *tmp_unisco_ops, const int tmp_ap_fd)
{
    int i;
    int dumpfile_fd = -1;
    size_t wc = 0;
    ...
    {
        if (qlog_exit_requested) return -1;
        ...
        if (strStartsWith(ql_exx00u_xml_data_defult.ql_cmd_table[i].cmd.type, "region"))
        {
            ...
            if ((i + 1) < ql_exx00u_xml_data_defult.ql_cmd_count && strStartsWith(ql_exx00u_xml_data_defult.ql_cmd_table[i + 1].cmd.type, "require"))
            {
                ...
                unisoc_exx00u_recv_cmd_deal(unisoc_exx00u_blue_screen_fd, pbuf, &rvalue, 1, NULL, tmp_unisco_ops, tmp_ap_fd);

                // check whether to perform dump
                if (!((rvalue & ql_exx00u_xml_data_defult.ql_cmd_table[i + 1].require.mask) == ql_exx00u_xml_data_defult.ql_cmd_table[i + 1].require.value)) continue;
            }
            ...
            if (access(blue_screen_logfile_dir, F_OK) && errno == ENOENT) mkdir(blue_screen_logfile_dir, 0755);
            ...
            uint8_t dump_save_buff[2048];
            memset(dump_save_buff, 0, sizeof(dump_save_buff));

            while (save_size < size_tmp)
            {
                if (qlog_exit_requested) return -1;

                wc = unisoc_exx00u_send_cmd_complete(unisoc_exx00u_blue_screen_fd, base_tmp, (size_tmp - save_size) > 0x0800 ? 0x0800 : (size_tmp - save_size), 1);
                if (wc != 13)
                {
                ...
                size_t tmp_rc = unisoc_exx00u_recv_cmd_deal(unisoc_exx00u_blue_screen_fd, pbuf, &rvalue, 0, dump_save_buff, tmp_unisco_ops, tmp_ap_fd);
                if (tmp_rc == 0) continue;

                save_size += tmp_rc;
                base_tmp += tmp_rc;

                wc = qlog_logfile_save(dumpfile_fd, dump_save_buff, tmp_rc);
                if (wc != tmp_rc)
                {
...
size_t unisoc_ec800g_send_cmd(int fd, uint8_t *buff, size_t size) { return qlog_poll_write(fd, buff, size, 1000); }

size_t unisoc_ec800g_recv_cmd(int fd, uint8_t *buff, size_t size) { return qlog_poll_read(fd, buff, size, 3000); }

size_t unisoc_ec800g_send_cmd_complete(int fd, uint32_t addr, uint32_t size, int type)
{
    uint8_t buf_send_query_tmp[32] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                      0x00, 0x00, 0x00, 0x00, 0x81, 0x95, 0xa2, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    uint8_t buf_send_query[64] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x81, 0x95, 0xa2, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    uint8_t buf_send_readBlock_tmp[32] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x81, 0x95, 0xa2, 0x80, 0x00, 0x04, 0x00, 0x00, 0x7e};
    uint8_t buf_send_readBlock[64] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                      0x00, 0x00, 0x00, 0x00, 0x81, 0x95, 0xa2, 0x80, 0x00, 0x04, 0x00, 0x00, 0x7e};
    uint8_t buf_send_read32_tmp[32] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                       0x20, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    uint8_t buf_send_read32[64] = {0x7e, 0x02, 0x00, 0x00, 0xab, 0x18, 0x00, 0x0b, 0x00, 0x03, 0xa0, 0x10, 0x00,
                                   0x20, 0x00, 0x00, 0x00, 0x00, 0x80, 0x00, 0x80, 0x01, 0x00, 0x00, 0x00, 0x7e};
    ...
    switch (type)
    {
        ...
        default: qlog_dbg("unisoc_ec800g_send_cmd_complete switch error\n");
    }
    ...
    do
    {
        ...
        switch (type)
        {
            case 0: // readBlock
            {
                ...
                {
                    if (((rc_valid_size - i) >= 20) &&
                        (pbuf_tmp[i] == 0x7e && pbuf_tmp[i + 7] == 0x0b && pbuf_tmp[i + 8] == 0x00 && pbuf_tmp[i + 9] == 0x03 && pbuf_tmp[i + 10] == 0xa0))
                    {
                        real_start_offset = i;
                        break;
                    }
            ...
            default: qlog_dbg("unisoc_ec800g_recv_cmd_deal switch error\n");
        }
...
int unisoc_ec800g_catch_blue_screen(uint8_t *pbuf, const char *logfile_dir)
{
    ...
    if (pbuf[0] == 0x7e && pbuf[5] == 0x19 && pbuf[6] == 0x00 && pbuf[7] == 0x0b && pbuf[8] == 0x00 && pbuf[9] == 0x03 && pbuf[10] == 0xa0 && pbuf[11] == 0x11 &&
        pbuf[12] == 0x00 && pbuf[13] == 0x00 && pbuf[14] == 0x00 && pbuf[15] == 0x00 && pbuf[16] == 0x00 && pbuf[17] == 0x00 && pbuf[18] == 0x00 && pbuf[19] == 0x00 &&
        pbuf[20] == 0x00 && pbuf[21] == 0x01 && pbuf[22] == 0x00 && pbuf[23] == 0x00 && pbuf[24] == 0x00 && pbuf[25] == 0x01 && pbuf[26] == 0x7e)
    {
        qlog_dbg("modem into blue screen dump ...\n");
        unisoc_ec800g_is_blue_screen = 1;
        sleep(5); //һ������blue dump��unisoc_exx00u_init_filter --> unisoc_ec800g_send_cmd_complete
                  //���ٷ��ͣ���ΪҪ�ȶ�rid_ec800g��EC600U����ȶ�rid�����Բ���Ҫ��ʱ
        ...
        if (pbuf[25] == 0xf3 && pbuf[26] == 0xf2 && pbuf[27] == 0xf1 && pbuf[28] == 0xf0 && rsize >= 36)
        {
            ...
            qlog_dbg("pbscfg->addr:%08x  pbscfg->size:%08x\n", pbscfg->addr, pbscfg->size);
        ...
        if (strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i].cmd.type, "region"))
        {
            ...
            else
            {
                    ...
                    !strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i].region.filename, "psram_phy"))
                {
                    ...
                    // qlog_dbg("ql_ec800g_xml_data_defult.ql_cmd_table[i].region.filename:%s\n",
                ...
                if ((i + 1) < ql_ec800g_xml_data_defult.ql_cmd_count && strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i + 1].cmd.type, "require"))
                {
                    ...
                    // check whether to perform dump
                    if (!((rvalue & ql_ec800g_xml_data_defult.ql_cmd_table[i + 1].require.mask) == ql_ec800g_xml_data_defult.ql_cmd_table[i + 1].require.value)) continue;
                    ...
                    if ((i + 2) < ql_ec800g_xml_data_defult.ql_cmd_count && strStartsWith(ql_ec800g_xml_data_defult.ql_cmd_table[i + 2].cmd.type, "require"))
                    {
                        ...
                        // check whether to perform dump
                        if (!((rvalue1 & ql_ec800g_xml_data_defult.ql_cmd_table[i + 2].require.mask) == ql_ec800g_xml_data_defult.ql_cmd_table[i + 2].require.value)) continue;
                    }
                }
...
{
    if (size <= 0 || NULL == buf || logfd <= 0) return size;
