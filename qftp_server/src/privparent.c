...
int capset(cap_user_header_t hdrp, const cap_user_data_t datap) { return syscall(__NR_capset, hdrp, datap); }
...
void handle_parent(session_t *sess)
{
    ...
    while (1)
    {
        ...
        switch (cmd)
        {
            case PRIV_SOCK_GET_DATA_SOCK: privop_pasv_get_data_sock(sess); break;
            case PRIV_SOCK_PASV_ACTIVE: privop_pasv_active(sess); break;
            case PRIV_SOCK_PASV_LISTEN: privop_pasv_listen(sess); break;
            case PRIV_SOCK_PASV_ACCEPT: privop_pasv_accept(sess); break;
        }
    }
}
...
void privop_pasv_get_data_sock(session_t *sess)
{
    ...
    // qserver_dbg("address ip: %s\n", ip);

    int data_fd = tcp_client(20);
    if (data_fd == -1)
    {
        qserver_dbg("tcp_client error data fd: %d\n", data_fd);
