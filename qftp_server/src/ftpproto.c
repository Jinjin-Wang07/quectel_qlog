...
void do_retr(session_t *sess)
{
    ...
    char path[1024];
    char t_path[1024]; // 存储路径的数组

    // 获取可执行程序的路径
    ssize_t len = readlink("/proc/self/exe", t_path, sizeof(t_path) - 1);
    if (len != -1)
    {
        char *tmp=strrchr(t_path,'/');
        *tmp='\0';
        //t_path[len] = '\0'; // 添加字符串结束符
        printf("Current executable path: %s\n", t_path);
    }
    else
    {
    ...
    char text[MAX_LINE * 2] = {0};
    if (sess->is_ascii)
    {
        sprintf(text, "Opening ASCII mode data connection %s (%lld bytes)", sess->cmd_arg, (long long)sbuf.st_size);
    }
    else
    {
        sprintf(text, "Opening BINARY mode data connection %s (%lld bytes)", sess->cmd_arg, (long long)sbuf.st_size);
    }
...
void do_stor(session_t *sess) { upload_common(sess, 0); }

void do_appe(session_t *sess) { upload_common(sess, 1); }
...
// 紧急模式数据接收
void do_abor(session_t *sess) { ftp_relply(sess, FTP_ABOR_NOCONN, "No transfer to ABOR."); }

void do_pwd(session_t *sess)
{
    char cur_dir[MAX_LINE + 1] = {0};
    char text[MAX_LINE + 2] = {0};
    char *ptr = getcwd(cur_dir, MAX_LINE);
    if (ptr == NULL) return;
    /**TODO */
...
void do_mkd(session_t *sess)
{
    ...
    {
        char text[MAX_LINE * 2 + 16] = {0};
        if (path[0] == '/')
        {
            sprintf(text, "%s created", path);
        }
        else
        {
            char cur_dir[MAX_LINE + 1] = {0};
            ...
            {
                sprintf(text, "%s%s created", cur_dir, path);
            }
            else
            {
                sprintf(text, "%s/%s created", cur_dir, path);
...
void do_syst(session_t *sess) { ftp_relply(sess, FTP_SYSTOK, "UNIX Type: L8"); }
...
// no operation
void do_noop(session_t *sess) { ftp_relply(sess, FTP_NOOPOK, "NOOP ok."); }
...
int list_common(session_t *sess, int detail)
{
    ...
    while ((dt = readdir(dir)) != NULL)
    {
        ...
        if (detail)
        {
            ...
            {
                char tmp[MAX_LINE] = {0};
                int res = readlink(dt->d_name, tmp, sizeof(tmp));
                if (res < 0) return -1;
                sprintf(buf + off, "%s -> %s\r\n", dt->d_name, tmp);
            }
            else
...
int lock_file_read(int fd) { return lock_internal(fd, F_RDLCK); }

int lock_file_write(int fd) { return lock_internal(fd, F_WRLCK); }
