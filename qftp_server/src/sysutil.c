...
int getlocalip(char *ip)
{
    ...
    if (ioctl(inet_sock, SIOCGIFADDR, &ifr) < 0) return -1;
...
int recv_fd(const int sock_fd)
{
    ...
    *p_fd = -1;
    ...
    recv_fd = *p_fd;
    if (recv_fd == -1) ERR_EXIT("no passed fd");
...
long get_time_usec() { return s_curr_time.tv_usec; }
