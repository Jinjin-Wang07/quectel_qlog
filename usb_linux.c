...
extern int qlog_only_read_pcie_or_usb_diag_log;
...
static int usbfs_is_kernel_driver_alive(int fd, int ifnum)
{
    ...
    {
        if (errno != ENODATA)
            qlog_dbg("%s ioctl USBDEVFS_GETDRIVER on interface %d failed, kernel driver may be "
                     "inactive\n",
                     __func__, ifnum);
        return 0;
    }
...
static int get_value_from_file(const char *fname, int base)
{
    ...
    if (fd <= 0)
    {
        if (errno != ENOENT) qlog_dbg("Fail to open %s,  errno: %d (%s)\n", fname, errno, strerror(errno));
        return -1;
    }
...
int ql_find_quectel_modules(void)
{
    ...
    if (NULL == usb_dir) return modules_num;

    while (NULL != (usb = readdir(usb_dir)))
    {
        char devpath[256] = {'\0'};
        if (usb->d_name[0] == '.' || usb->d_name[0] == 'u') continue;
        ...
        if ((ql_dev.idVendor == 0x3763 && ql_dev.idProduct == 0x3c93) || (ql_dev.idVendor == 0x3c93 && ql_dev.idProduct == 0xffff))
        {
            ...
            {
                qlog_dbg("The state grid module needs to specify the module, like -g EC200T\n");
                return modules_num;
            }
        }
        ...
        if ((ql_dev.idVendor == 0x05c6 && ql_dev.idProduct == 0x9003)               // UC15
            ...
            || (ql_dev.idVendor == 0x3625 && ql_dev.idProduct == 0x0004)            // EG060K
            || (ql_dev.idVendor == 0x3625 && ql_dev.idProduct == 0x0005)            // EG060K
            ...
            || drv_is_asr(ql_dev.idProduct, ql_dev.idVendor)                        // asr
        ...
        {
            if (drv_is_asr(ql_dev.idProduct, ql_dev.idVendor))
            {                                                   // ASR
            ...
            else if (drv_is_unisoc(ql_dev.idProduct, ql_dev.idVendor))
            {                                                   // unisoc
            ...
            else if (drv_is_unisoc_exx00u(ql_dev.idProduct, ql_dev.idVendor))
            {                                        // unisoc ECx00U or EGx00U
            ...
            else if (drv_is_mtk(ql_dev.idProduct, ql_dev.idVendor))
            { // mtk RM500K / AG568N
                ...
                else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x7008)
                {
                    ql_dev.dm_intf.bInterfaceNumber = 5;
                }
                else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x7006)
                {
                    ql_dev.dm_intf.bInterfaceNumber = 5;
                }
                else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x7003)
                {
                    ql_dev.dm_intf.bInterfaceNumber = 5;
                }
            }
            else if (drv_is_eigen(ql_dev.idProduct, ql_dev.idVendor))
            {                                                                // eigencomm
            ...
            else if (ql_dev.idVendor == 0x2c7c && (ql_dev.idProduct == 0x0457 || ql_dev.idProduct == 0x0458 || ql_dev.idProduct == 0x0452))
            {
                ql_dev.general_intf.bInterfaceNumber = 13; // qdss_intf
                ql_dev.general_type = MDM_QDSS;

                ql_dev.third_intf.bInterfaceNumber = 14; // third_intf
                ql_dev.third_type = 1;
            ...
            else if (ql_dev.idVendor == 0x3625 && (ql_dev.idProduct == 0x0004 || ql_dev.idProduct == 0x0005))
            { // mdm
                if (!qlog_only_read_pcie_or_usb_diag_log)
                {
                    ql_dev.general_intf.bInterfaceNumber = 12; // qdss_intf
        ...
        // Adapt to capture dump(ql_dev.bNumInterfaces == 1) and capture log(ql_dev.bNumInterfaces >
        // 1) to specify DM port
            ...
            || (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x030d)) // EM05G-FCCL Laptop
        {
            ...
            if ((ql_dev.bNumInterfaces > 1) && (ql_dev.idProduct == 0x0128 || ql_dev.idProduct == 0x6008 || ql_dev.idProduct == 0x6009))
            {
                if (!qlog_only_read_pcie_or_usb_diag_log)
                {
        ...
        else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x0133)
        {
            ql_dev.dm_intf.bInterfaceNumber = 2;
        }
        else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x0134)
        {
            ql_dev.dm_intf.bInterfaceNumber = 0;
        }
        else if (ql_dev.idVendor == 0x2c7c && ql_dev.idProduct == 0x0125 && g_module_from_ecm_2_rndis)
        {
            ql_dev.dm_intf.bInterfaceNumber = 2;
        }

        // module from ecm to rndis
        if (g_module_from_ecm_2_rndis > 0)
        {
            ql_dev.dm_intf.bInterfaceNumber = 2;
        }
    ...
    _rescan_dm:
        ...
        if (access(devpath, F_OK)) continue;
        ...
        else if (ql_dev.general_intf.bInterfaceNumber != 0xFF && ql_dev.general_type == MDM_QDSS)
        {
            ...
            {
                ql_find_usb_interface_info(devpath, &ql_dev.general_intf);
                if (ql_dev.general_intf.bInterfaceClass != 0xff || ql_dev.general_intf.bInterfaceSubClass != 0xff || ql_dev.general_intf.bInterfaceProtocol != 0x70 ||
                    ql_dev.general_intf.bNumEndpoints != 1 || ql_dev.general_intf.ep_out != 0xff || ql_dev.general_intf.ep_in == 0xff)
                {
        ...
        else if (ql_dev.third_intf.bInterfaceNumber != 0xFF && ql_dev.third_type == 1)
        {
            ...
            {
                ql_find_usb_interface_info(devpath, &ql_dev.third_intf);
                if (ql_dev.third_intf.bInterfaceClass != 0xff || ql_dev.third_intf.bInterfaceSubClass != 0xff || ql_dev.third_intf.bInterfaceProtocol != 0x80 ||
                    ql_dev.third_intf.bNumEndpoints != 1 || ql_dev.third_intf.ep_out != 0xff || ql_dev.third_intf.ep_in == 0xff)
                {
        ...
        qlog_dbg("Find [%d] idVendor=%04x, idProduct=%04x, bNumInterfaces=%d, ttyDM=%s, "
                 "ttyGENERAL=%s, ttyTHIRD=%s, busnum=%03d, dev=%03d, usbdevice_pah=%s\n",
                 modules_num, ql_dev.idVendor, ql_dev.idProduct, ql_dev.bNumInterfaces, ql_dev.ttyDM, ql_dev.ttyGENERAL, ql_dev.ttyTHIRD, ql_dev.busnum, ql_dev.devnum,
                 ql_dev.usbdevice_pah);

        if (modules_num < MAX_USB_DEV) s_usb_device_info[modules_num++] = ql_dev;
    }
...
#define MKDEV(__ma, __mi) (((__ma & 0xfff) << 8) | (__mi & 0xff) | ((__mi & 0xfff00) << 12))
    ...
    if (usbfs_is_kernel_driver_alive(usbfd, intf)) usbfs_detach_kernel_driver(usbfd, intf);
