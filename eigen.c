...
const uint8_t eigen_log_time_start[8] = {0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xAA};

const uint8_t eigen_log_time_end[8] = {0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xBA, 0xAA};
...
int SyncStart(void)
{
    ...
    {
        qlog_dbg("%s wait_tty_echo_not_busy faileed\n", __func__);
        return -1;
    }
    ...
    for (nRetry = 0; nRetry < MAX_RETRY_TIME; ++nRetry)
    {
        ...
        if (err_code != UE_OK)
        {
            qlog_dbg("%s SendRamDumpAck error\n", __func__);
            continue;
        }
        else
        {
            qlog_dbg("%s SendRamDumpAck success\n", __func__);
        }
        ...
        if (err_code == UE_OK)
        {
            qlog_dbg("%s WaitRamDumpAckAndData success\n", __func__);
            break;
        }
        else
        {
            qlog_dbg("%s WaitRamDumpAckAndData fail\n", __func__);
        }
    }
    ...
    for (nRetry = 0; nRetry < MAX_RETRY_TIME; ++nRetry)
    {
        ...
        if (err_code == UE_OK)
        {
            qlog_dbg("%s WaitRamDumpAckAndData success -1\n", __func__);
            break;
        }
    }
...
int SendGetInfoUsb(void)
{
    ...
    nCount = sizeof(dataReq.Command) + sizeof(dataReq.CID) + sizeof(dataReq.Sequence) + sizeof(dataReq.NCID) + sizeof(dataReq.Length);
...
int GetDumpDataUsb(uint32_t nTotalData, uint32_t nReadAddr, int bFinish)
{
    ...
    while (nReadLen < nTotalData && nTotalData > 0 && nReadTime < MAX_RETRY_TIME / 2)
    {
        ...
        while (nPktIndex < newReadData.ReadLen + 12 && nRetry < MAX_RETRY_TIME / 4)
        {
            ...
            if (dataRsp->Length > newReadData.ReadLen) break;
        }
        ...
        if (err_code == UE_OK)
        {
            ...
            if (wNewCRC == (uint16_t)dwDumpDataCRC)
            {
                ...
                if (nTotalData - nReadLen < dwUsbMaxDataLen) newReadData.ReadLen = nTotalData - nReadLen;
                nReadTime = 0;
            }
            else
            {
...
int enigen_catch_dump(uint8_t* pbuf, ssize_t size, const char* logfile_dir, const char* (*qlog_time_name)(int))
{
    ...
    for (i = 0; i < RDReqCell_count; i++)
    {
        ...
        if (i + 1 < RDReqCell_count) PaddingDumpFile(RDReqCell[i + 1].ReadDataAddr - (RDReqCell[i].ReadLen + RDReqCell[i].ReadDataAddr));
    }
