...
ssize_t asr_send_cmd(int fd, const unsigned char *buf, size_t size)
{
    ...
    while (wc < size)
    {
        ...
        if (cmd_len > (size - wc)) break;

        if (qlog_poll_write(fd, buf + wc, cmd_len, 1000) != (ssize_t)cmd_len) break;

        while (rx_wait-- > 0)
        {
            if (g_rx_log_count != rx_count) break;
            usleep(1 * 1000);
        }
...
#define _my_write(_cmd)                                           \
    do                                                            \
    {                                                             \
        if (asr_send_cmd(fd, _cmd, sizeof(_cmd)) != sizeof(_cmd)) \
        {                                                         \
            goto my_write_fail;                                   \
        }                                                         \
    } while (0)
#define _my_write_tmp(_cmd)                                       \
    do                                                            \
    {                                                             \
        if (asr_send_cmd(fd, _cmd, sizeof(_cmd)) != sizeof(_cmd)) \
        {                                                         \
    ...
    for (t = 0; t < 30; t++)
    {
        ...
        // }
        _my_write_tmp(ACATReady);
        qlog_dbg("send ACATReady t [%d]-[%s]-[%d]\r\n", t, __func__, __LINE__);
    }
...
static int asr_logfile_init(int logfd, unsigned logfile_seq)
{
    ...
    if (nbytes != (ssize_t)size) qlog_dbg("write %zd/%zd, errno: %d (%s)\n", nbytes, size, errno, strerror(errno));
...
#define data_max 8192
    ...
    if (!data_buf) data_buf = (uint8_t *)malloc(data_max);
    if (!data_buf) return;
...
_start:
    ...
    while ((offset + sizeof(struct DBVerInfo)) < data_len)
    {
        ...
        if (g_dwUETime && dwUETime && dwUETime != 0xFFFFFFFF && abs((int)(dwUETime - g_dwUETime)) > (4 * 1000 * 1000))
        {
        ...
        if (0x1b == len && info->version[0] == '0' && info->version[1] == 'x' && pbuf[len - 1] == 0 && DIAG_MODID == qlog_le16(info->wModID) &&
    ...
    data_len -= offset;
    if (data_len && offset) memmove(data_buf, data_buf + offset, data_len);
...
static size_t asr_logfile_save(int logfd, const void *buf, size_t size)
{
    ...
    if (!g_query_version_done) asr_parser_dbversion(buf, size);
